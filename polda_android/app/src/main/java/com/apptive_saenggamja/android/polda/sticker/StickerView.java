...
public class StickerView extends FrameLayout {
  ...
  public boolean tempStateExist;

  //가이드 나오게 편집한부분
  public void setEditGuide(boolean show){
    Log.d("stickerView",Boolean.toString(show));
    this.showIcons=show;
    this.showBorder=show;
    Log.d("stickerView",Boolean.toString(showIcons));
  }
...
//  public List<Sticker> getStickerState(){
//    List<Sticker> StickerList;
//    StickerList=
//  }

  public void restoreStickers(List<Sticker> stickerList){
  ...
  //  get{Log.d("sticker",stickers.toString()};
  ...
  protected void handleCurrentMode(@NonNull MotionEvent event) {
    Log.d("matrix","handleCurrentMode!!");
    switch (currentMode) {
  ...
  @Override protected void onSizeChanged(int w, int h, int oldW, int oldH) {
    Log.d("create","StickerView onSizeChanged");
    super.onSizeChanged(w, h, oldW, oldH);
    for (int i = 0; i < stickers.size(); i++) {
      Sticker sticker = stickers.get(i);


//이거 있으면 tempSticker가 가운데로 가버림
//      if (sticker != null) {
...
//      }
  ...
  public StickerView addSticker(@NonNull final Sticker sticker,
    ...
    Log.d("matrix","add sticker2!!");

    if (ViewCompat.isLaidOut(this)) {
      Log.d("create","view existed");
      addStickerImmediately(sticker, position);
    } else {
      Log.d("create","view  not existed");
      post(new Runnable() {
  ...
  protected void addStickerImmediately(@NonNull Sticker sticker, @Sticker.Position int position) {
    ...
    Log.d("matrix","add sticker immediately!!");
  ...
  protected void setStickerPosition(@NonNull Sticker sticker, @Sticker.Position int position) {
    Log.d("matrix","setStickerPosition!!");
    float width = getWidth();
    float height = getHeight();
